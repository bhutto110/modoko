// lib/whm.js - Call PHP proxy instead of WHM directly
const PROXY_URL = 'https://your-php-server.com/whm-proxy.php'; // Change this!

export async function fetchWHM(host, token, endpoint) {
  const proxyUrl = `${PROXY_URL}?host=${encodeURIComponent(host)}&token=${encodeURIComponent(token)}&endpoint=${encodeURIComponent(endpoint)}`;
