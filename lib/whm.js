// Use native fetch with SSL bypass via environment variable
// Set NODE_TLS_REJECT_UNAUTHORIZED=0 in Vercel env vars
...
export async function fetchWHM(host, token, endpoint) {
  ...
  try {
    ...
    console.error(`WHM API Error for ${host}/${endpoint}:`, error.message);
    throw error;
  }
}
...
export async function getServiceStatus(host, token) {
  ...
  return data.data?.services || data.result?.data?.services || [];
}
...
export async function getSystemInfo(host, token) {
  ...
  try {
    ...
    if (load.data) {
      info.load1 = formatLoad(load.data.one || load.data['1']);
      info.load5 = formatLoad(load.data.five || load.data['5']);
      info.load15 = formatLoad(load.data.fifteen || load.data['15']);
